...
async function main(): Promise<void> {
  ...
  const challengeSeeds = [
    ...
    {
      category: "portrait",
      coverUrl: "/images/challenges/available-light-portrait.png",
      slug: "available-light-portrait",
      status: "ACTIVE" as const,
    },
    {
      category: "landscape",
      coverUrl: "/images/challenges/wild-weather.png",
      slug: "weather-and-land",
  ...
  for (const challengeSeed of challengeSeeds) {
    const category = await prisma.category.findUniqueOrThrow({
      where: { slug: challengeSeed.category },
    });
    await prisma.challenge.upsert({
      create: {
        categoryId: category.id,
        coverUrl: challengeSeed.coverUrl,
