...
type SocialLinksForm = Record<SocialPlatformId, SocialLinkRecord>;
...
const emptySocialLinksForm: SocialLinksForm = {
  adobe: { url: "" },
  artstation: { url: "" },
  behance: { url: "" },
  facebook: { url: "" },
...
const socialPlatforms: readonly {
  ...
  },
  {
    ...
    mark: "A",
    placeholder: "username.artstation.com"
  },
  {
    hosts: ["adobe.com", "myportfolio.com"],
    id: "adobe",
    key: "social.adobe",
    mark: "Ad",
    placeholder: "portfolio.adobe.com/username"
  },
  {
    hosts: ["behance.net"],
    id: "behance",
...
export function HomeClient({ initialSection, locale }: HomeClientProps): ReactNode {
  ...
  const [socialLinksForm, setSocialLinksForm] = useState<SocialLinksForm>(emptySocialLinksForm);
  ...
  useEffect(() => {
    if (!currentProfile) {
      ...
      setSocialLinksForm(emptySocialLinksForm);
      return;
    }
    ...
    setSocialLinksForm(createSocialLinksForm(currentProfile.socialLinks));
  ...
  async function handleSocialAvatarChange(
    ...
    try {
      const avatarUrl = await readFileAsDataUrl(file);
      setSocialLinksForm((current) => ({
        ...current,
        [platformId]: { ...current[platformId], avatarUrl }
      }));
      setGlobalFeedback({ kind: "success", text: t("profile.socialAvatarSaved") });
    } catch {
      setGlobalFeedback({ kind: "error", text: t("photo.invalid") });
    }
  }
  ...
  function updateSocialLink(platformId: SocialPlatformId, url: string): void {
    setSocialLinksForm((current) => ({
      ...current,
      [platformId]: { ...current[platformId], url }
    }));
  }
  ...
  function saveProfile(event: FormEvent<HTMLFormElement>): void {
    ...
    const socialLinks = normalizeSocialLinks(socialLinksForm);
    if (!socialLinks) {
      setGlobalFeedback({ kind: "error", text: t("profile.socialInvalid") });
      return;
    }
          ...
          <aside className="profile-edit">
            <form className="auth-form" onSubmit={saveProfile}>
              ...
              <fieldset className="social-editor">
                ...
                <p>{t("profile.socialsIntro")}</p>
                <div className="social-editor-list">
                  {socialPlatforms.map((platform) => {
                    const socialLink = socialLinksForm[platform.id];
                    const avatarInputId = `social-avatar-${platform.id}`;

                    return (
                      <div className="social-editor-row" key={platform.id}>
                        <span className={`social-brand-mark is-${platform.id}`}>{platform.mark}</span>
                        <label className="social-url-field" htmlFor={`social-url-${platform.id}`}>
                          <span>{t(platform.key)}</span>
                          <input
                            id={`social-url-${platform.id}`}
                            onChange={(event) => {
                              updateSocialLink(platform.id, event.target.value);
                            }}
                        ...
                        <label
                          aria-label={t("profile.socialAvatar")}
                          className="social-avatar-picker"
                          htmlFor={avatarInputId}
                          title={t("profile.socialAvatar")}
                        >
                        ...
                        <input
                          ...
                          id={avatarInputId}
                          onChange={(event) => {
                            void handleSocialAvatarChange(platform.id, event);
                          }}
                          type="file"
                        />
                      </div>
...
function createSocialLinksForm(socialLinks?: SocialLinks): SocialLinksForm {
  return {
    adobe: socialLinks?.adobe ?? { url: "" },
    artstation: socialLinks?.artstation ?? { url: "" },
    behance: socialLinks?.behance ?? { url: "" },
    facebook: socialLinks?.facebook ?? { url: "" },
    instagram: socialLinks?.instagram ?? { url: "" }
  };
}

function normalizeSocialLinks(form: SocialLinksForm): SocialLinks | null {
  const normalized: SocialLinks = {};

  for (const platform of socialPlatforms) {
    const socialLink = form[platform.id];
    const rawUrl = socialLink.url.trim();
    ...
    try {
      ...
      const hostname = parsedUrl.hostname.toLocaleLowerCase().replace(/^www\./, "");
      const matchesPlatform = platform.hosts.some(
        (allowedHost) => hostname === allowedHost || hostname.endsWith(`.${allowedHost}`)
      );

      if (parsedUrl.protocol !== "https:" || !matchesPlatform) {
        return null;
      }

      normalized[platform.id] = {
        avatarUrl: socialLink.avatarUrl,
