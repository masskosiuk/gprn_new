...
const locationPins: readonly LocationPin[] = [
  { id: "paris", key: "map.location.paris", x: 47, y: 39 },
  { id: "kyiv", key: "map.location.kyiv", x: 55, y: 36 },
  { id: "tokyo", key: "map.location.tokyo", x: 82, y: 46 },
  { id: "reykjavik", key: "map.location.reykjavik", x: 38, y: 23 },
  { id: "lisbon", key: "map.location.lisbon", x: 43, y: 47 },
  { id: "marrakech", key: "map.location.marrakech", x: 45, y: 54 }
];
...
export function HomeClient({ initialSection, locale }: HomeClientProps): ReactNode {
  ...
  const allPhotos = [...uploadedPhotos, ...curatedPhotos];
  const publicPhotos = allPhotos.filter((photo) => photo.published || photo.isMine);
  ...
  async function handlePhotoChange(event: ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    if (currentProfile && !currentProfile.avatarUrl) {
      setAccount({
        ...currentProfile,
        avatarUrl: uploadedPhoto.src,
        coverUrl: uploadedPhoto.src
      });
    }
  }
  ...
  async function handleAuthSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (authMode === "register") {
      ...
      const newAccount: AccountRecord = {
        availableForHire: true,
        avatarUrl: selectedUploadedPhoto?.src,
        battles: 0,
        bio: t("profile.defaultBio"),
        coverUrl: selectedUploadedPhoto?.src,
        email,
        followers: 128,
        following: 42,
  ...
  function setPhotoAsAvatar(photo: PhotoRecord): void {
    if (!currentProfile) {
      openAuth("login");
    ...
    return (
      <>
        <section className="page-intro">
          <div>
            <span className="eyebrow">{t("app.shortName")}</span>
  ...
  function renderUploadPanel(): ReactNode {
    return (
      <aside className="upload-panel">
        ...
        {selectedUploadedPhoto ? (
          <div className="selected-file">
            <img alt={getPhotoTitle(selectedUploadedPhoto, locale)} src={selectedUploadedPhoto.src} />
            <div>
              <strong>{getPhotoTitle(selectedUploadedPhoto, locale)}</strong>
              <span>{selectedUploadedPhoto.fileName}</span>
              <small>{selectedUploadedPhoto.sizeLabel}</small>
            </div>
          </div>
        ) : (
          <p className="helper-message">{t("photo.empty")}</p>
        )}

        {photoFeedback ? <p className={`status-message ${photoFeedback.kind}`}>{photoFeedback.text}</p> : null}
      </aside>
    );
  }
        ...
        <div className="main-column">
          <div className="map-board">
            ...
            {locationPins.map((location) => (
              <button
                className={`map-pin${locationFilter === location.id ? " is-active" : ""}`}
                key={location.id}
                onClick={() => {
                  setLocationFilter(location.id);
                }}
                style={{ "--x": `${location.x}%`, "--y": `${location.y}%` } as CSSProperties}
                type="button"
              >
          ...
          <aside className="profile-edit">
            ...
            <section className="account-panel" aria-labelledby="privacy-title">
              ...
              <div className="connection-summary">
                ...
                <span>{t("privacy.noConnections")}</span>
              </div>
            </section>
          </aside>
        </div>
