...
const socialHosts = {
  adobe: ["adobe.com", "myportfolio.com"],
...
export class ProfilesService {
  ...
  async updateMine(user: CurrentUser, body: unknown) {
    ...
    if (coverPhotoId) {
      ...
      const coverAsset = photo?.assets.find((asset) => asset.type === "DISPLAY" || asset.type === "THUMBNAIL");
      if (!coverAsset) {
        throw new BadRequestException({ code: "PROFILE_COVER_INVALID", message: "Choose one of your processed photos." });
      }
    ...
    try {
      const profile = await prisma.$transaction(async (tx) => {
        ...
        const saved = await tx.profile.update({
          data: { avatarAssetKey, bio, coverAssetKey, displayName, socialLinks, username, visibility, websiteUrl },
          where: { userId: user.id }
        });
  ...
  private serializeProfile(profile: ProfileRecord) {
    return {
      ...
      bio: profile.bio,
      coverUrl: profile.coverAssetKey ? publicAssetUrl(this.env, profile.coverAssetKey) : null,
...
function parseSocialLinks(value: unknown): Prisma.InputJsonValue | undefined {
  if (value === undefined) return undefined;

  const input = asRecord(value, "PROFILE_SOCIAL_LINKS_INVALID");
  const result: Record<string, { avatarUrl?: string; url: string }> = {};

  for (const [platform, hosts] of Object.entries(socialHosts)) {
    const rawLink = input[platform];
    if (rawLink === undefined || rawLink === null) continue;

    const link = asRecord(rawLink, "PROFILE_SOCIAL_LINK_INVALID");
    const url = optionalString(link, "url");
    const avatarUrl = optionalString(link, "avatarUrl");

    if (!url || !isAllowedSocialUrl(url, hosts)) {
      throw new BadRequestException({
        code: "PROFILE_SOCIAL_LINK_INVALID",
        field: platform,
        message: `Enter a valid HTTPS ${platform} profile URL.`
      });
    }

    if (avatarUrl && !isSafeImageUrl(avatarUrl)) {
      throw new BadRequestException({
        code: "PROFILE_SOCIAL_AVATAR_INVALID",
        field: platform,
        message: "Social avatar URL must use HTTPS."
      });
    }

    result[platform] = avatarUrl ? { avatarUrl, url } : { url };
  }

  return result;
}

function isAllowedSocialUrl(value: string, hosts: readonly string[]): boolean {
  try {
