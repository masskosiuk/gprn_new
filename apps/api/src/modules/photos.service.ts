...
const allowedLocationVisibility = [
  "EXACT",
  "APPROXIMATE",
  "CITY",
  "COUNTRY",
...
interface PhotoRecord {
  readonly _count?: {
    ...
    readonly moodboardItems: number;
    readonly reviews: number;
    readonly savedBy: number;
  };
  readonly id: string;
...
export class PhotosService {
  ...
  async addFromDevice(
    ...
    const storageKeys = createPhotoStorageKeys(
      user.id,
      photoId,
      input.fileName,
    );
      ...
      const createdPhoto = await tx.photo.create({
        data: {
          ...
          versions: {
            create: {
              changeSummary:
                "Original upload stored and analyzed for MVP provenance.",
  ...
  async listMine(
    user: CurrentUser,
  ): Promise<{ photos: ReturnType<PhotosService["toPhotoResponse"]>[] }> {
  ...
  async listPublic(): Promise<{
    photos: ReturnType<PhotosService["toPhotoResponse"]>[];
  }> {
    const photos = await prisma.photo.findMany({
      include: photoInclude,
      orderBy: {
  ...
  private async tryCreateBattle(
    ...
    const entrantRating = await tx.rating.findUnique({
      where: {
        userId_scope_scopeKey: {
          scope: "GLOBAL",
          scopeKey: "global",
    ...
    const opponent = candidates.sort(
      (left, right) =>
        Math.abs(
          (left.owner.ratings[0]?.rating ?? 1500) -
            (entrantRating?.rating ?? 1500),
        ) -
        Math.abs(
          (right.owner.ratings[0]?.rating ?? 1500) -
            (entrantRating?.rating ?? 1500),
        ),
    )[0];
  ...
  private toPhotoResponse(photo: PhotoRecord) {
    ...
    return {
      ...
      counts: {
        ...
        bookmarks: photo._count?.savedBy ?? 0,
        challenges: photo._count?.challengeEntries ?? 0,
        comments: photo._count?.comments ?? 0,
        disputes: photo._count?.disputesAsSubject ?? 0,
        likes: photo._count?.likes ?? 0,
        moodboards: photo._count?.moodboardItems ?? 0,
        reviews: photo._count?.reviews ?? 0,
      },
        ...
        ? {
            ...
            hasExif:
              ...
              "hasExif" in photo.metadata.exifJson
                ? Boolean(
                    (photo.metadata.exifJson as { readonly hasExif?: unknown })
                      .hasExif,
                  )
                : false,
      ...
      status: photo.status,
      storage: {
        displayBytes: displayAsset
          ? bigintToString(displayAsset.byteSize)
          : null,
      },
      title: photo.title,
      ...
      checks: [
        {
          ...
          status: photo.provenance?.originalFileDetected
            ? "SUPPORTED"
            : "UNAVAILABLE",
        },
        {
          ...
          status: photo.provenance?.metadataDetected
            ? "SUPPORTED"
            : "UNAVAILABLE",
        },
        {
          ...
          status: photo.provenance?.captureDateDetected
            ? "SUPPORTED"
            : "UNAVAILABLE",
        },
        {
...
function parsePublishPhotoInput(body: unknown): PublishPhotoInput {
  ...
  return {
    locationVisibility: optionalEnum(
      record,
      "locationVisibility",
      allowedLocationVisibility,
    ),
    visibility:
      optionalEnum(record, "visibility", allowedVisibility) ?? "PUBLIC",
  };
...
function createPhotoStorageKeys(
  userId: string,
  photoId: string,
  fileName: string,
): {
