...
```text
https://photoapp.metarp.top/api/v1/social-connections/facebook/callback
https://photoapp.metarp.top/api/v1/social-connections/instagram/callback
```

После этого добавьте выданные значения в `/opt/gprn/.env.production`:
...
Instagram использует официальный Instagram Login и поддерживает
профессиональные аккаунты (Business/Creator). Секреты не добавляйте в
репозиторий или архив. После изменения env пересоздайте контейнер API.
